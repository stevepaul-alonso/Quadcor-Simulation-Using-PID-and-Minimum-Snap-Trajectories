...
from neat import Checkpointer as cp
import csv
import os
import math
...
def simulation(t5,Pa,Va,Pb,Vb, WP_A, WP_B, T, output):
	...
	cost = (energy_A)**2 + (energy_B)**2 + cons
	##print(min_sep)
	##plt.clf()
...
def run(config_file):
	...
	#par = neat.ParallelEvaluator(2,fitness_function)
	...
	pe = neat.ParallelEvaluator(num_workers, fitness_function)
	winner = p.run(pe.evaluate, 35)
	...
	for i in range(500):
		...
		#print(wrt)
		with open('result2.csv','a') as fin:	
